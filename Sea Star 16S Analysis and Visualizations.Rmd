...
fig1A<-ggplot(nonegs_mapfile, aes(x = ExtractionKitAbb, y = dna_amount_ng, color= ExtractionKitAbb)) +
  geom_boxplot(alpha=0.3,outlier.shape = NA) +
  labs(y = "DNA Yield (ng)", x = "") +
  geom_jitter(aes(color=ExtractionKitAbb, shape=Species), width=0.35, alpha=0.5) +
  theme_classic2() + 
...
fig1B<-ggplot(pcr_success, aes(x = kit, y = count, fill=factor(group, levels=c("Never amplified","Amplified with troubleshooting", "Amplified first attempt")))) + 
  geom_bar(stat = "identity", alpha=0.7) + 
   theme_classic2() +
  scale_fill_manual(values=c("firebrick", "gold1", "cyan4"), name="Amplification Success") +
  xlab("") +
  ylab("No. Samples")
...
ordination_S1<-plot_ordination(sswd.decontam, pcoa_all, color = "ExtractionKitAbb", shape = "Species") + theme_classic2() + geom_point(size = 2.5) + scale_color_manual(values=c("red", "#332288", "#6699CC", "#882255", "#CC6677", "limegreen")) + geom_text_repel(aes(label=row.names(sample_data(sswd.decontam))), size = 2.5) + scale_shape_manual(values=c(19,4,2,15)) + theme(text=element_text(family="sans")) + labs(color = "Extraction Kit", shape="Sample Type")
...
Fig2A <-ggplot(ss_ba_table, aes(x = ExtractionKitAbb, y = estimate, color= ExtractionKitAbb)) +
  geom_boxplot(alpha=0.3, outlier.shape=NA) +
  labs(y = "Breakaway Richness", x = "") +
  geom_jitter(aes(color= ExtractionKitAbb, shape=Species), width=0.35, alpha=0.5) + theme_classic2() +
  scale_color_manual(values=c("#332288", "#6699CC", "#882255", "#CC6677", "limegreen"))  + 
  theme(legend.text = element_text(face = "italic")) +
  guides(color = "none") 
...
FigS1A <-ggplot(ss_ba_table, aes(x = Species, y = estimate, color= Species)) +
  geom_boxplot(alpha=0.3, outlier.shape=NA) +
  labs(y = "Breakaway Richness", x = "", shape = "Extraction Kit") +
  geom_jitter(aes(shape=ExtractionKitAbb), width=0.35, alpha=0.5) + theme_classic2() +
...
#ANOVA with Derm and Pisaster combined (all data)
all_anova<-aov(estimate ~ ExtractionKitAbb, data = combined_richness)
summary(all_anova)

#testing anova assumptions
#homogeneity of variances
bartlett.test(estimate ~ ExtractionKitAbb, data = combined_richness)
#p-value=0.9541 so passes homogeneity test

#inspect the residuals for normality using plots and shapiro.test
all_resid<-residuals(all_anova)
plot(all_anova, 1)
hist(all_resid)
shapiro.test(all_resid)
#p-value = 0.149 so passes normality test

#Running ANOVAs looking at effect of extraction kit on richness within each species separately
...
#p-value=0.869 so passes homogeneity test
...
#p-value = 0.4135 so passes normality test
...
#p-value=0.2031 so passes homogeneity test
...
#p-value = 0.9777 so passes normality test
...
#p-value=0.243 so passes homogeneity test
...
#p-value = 0.3774 so passes normality test
...
Fig2B <-ggplot(ss_div_shn, aes(x = ExtractionKitAbb, y = estimate, color= ExtractionKitAbb)) +
  geom_boxplot(alpha=0.3, outlier.shape=NA) +
...
FigS1B <-ggplot(ss_div_shn, aes(x = Species, y = estimate, color= Species)) +
  geom_boxplot(alpha=0.3, outlier.shape=NA) +
  labs(y = "Shannon Diversity", x = "", shape = "Extraction Kit") +
  geom_jitter(aes(shape=ExtractionKitAbb), width=0.35, alpha=0.5) + theme_classic2() +
  scale_color_manual(values=c("firebrick","darkorchid4"))  +
  guides(color = "none") + 
...
ggsave(filename = "Figure S1.pdf", plot = figureS1)
ggsave(filename = "Figure S1.jpg", plot = figureS1)
...
#ANOVA with Derm and Pisaster combined (all data)
all_anova_shan<-aov(estimate ~ ExtractionKitAbb, data = combined_shannon)
summary(all_anova_shan)

#testing anova assumptions
#homogeneity of variances
bartlett.test(estimate ~ ExtractionKitAbb, data = combined_shannon)
#p-value=0.9708 so passes homogeneity test

#inspect the residuals for normality using plots and shapiro.test
all_resid_shan<-residuals(all_anova_shan)
plot(all_anova_shan, 1)
hist(all_resid_shan)
shapiro.test(all_resid_shan)
#p-value = 0.001361 so does NOT pass normality test

#Instead we can used the non-parametric kruskal test

kruskal.test(estimate ~ ExtractionKitAbb, data = combined_shannon)
#p-value = 0.8853

#ANOVAs looking at effect of extraction kit on diversity within each species separately
...
#p-value = 0.001523 so does not pass normality test
...
##p-value=0.06 so just passes homogeneity test
...
#p-value = 0.4095 so passes normality test, can use ANOVA in this case for Derm Shannon diversity test
...
#p-value= 0.1727 so passes homogeneity test

#inspect the residuals for normality using plots and shapiro.test
sp_resid_shan<-residuals(species_anova_shan)
plot(species_anova_shan, 1)
hist(sp_resid_shan)
shapiro.test(sp_resid_shan)
#p-value = 0.03024 so does NOT pass normality test

#using a non-parametric alternative to test effect of sea star species on diversity

kruskal.test(estimate ~ Species, data = combined_shannon)
#p-value = 0.00355
...
fig3<-plot_ordination(seastar_filt_clean, pcoa, color = "ExtractionKitAbb", shape = "Species") + theme_classic() + geom_point(aes(color =ExtractionKitAbb), alpha = 0.7, size = 4) + scale_color_manual(values=c("#332288", "#6699CC", "#882255", "#CC6677", "limegreen")) + labs(color = "Extraction Kit") +
...
#saving Figure 3
ggsave(filename = "Figure 3.pdf", plot = fig3)
ggsave(filename = "Figure 3.jpg", plot = fig3)
